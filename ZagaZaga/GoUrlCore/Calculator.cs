...
namespace ZagaZaga.GoUrlCore
{
    public static class Calculator
    {
        ...
        public static string md512(string s)
        {
            byte[] hash = Encoding.ASCII.GetBytes(s);
        ...
        public static string icrc32(string str)
        {
            uint inp = crc32(str);
            uint int_max = (uint)(Math.Pow(2, 31) - 1);
            uint output = 0;
            if (inp > int_max)
            {
                output = inp - int_max * 2 - 2;
            }
            else
            {
                output = inp;
        ...
        public static TimeSpan ConvertPeriod(string period)
        {
            if (period.Contains("MINUTE"))
            {
                return TimeSpan.FromMinutes(double.Parse(period.Split(' ')[0]));
            }
            if (period.Contains("HOUR"))
            {
                return TimeSpan.FromHours(double.Parse(period.Split(' ')[0]));
            }
            if (period.Contains("DAY"))
            {
                return TimeSpan.FromDays(double.Parse(period.Split(' ')[0]));
            }
